...
function get_curl($url, $post=0, $referer=0, $cookie=0, $header=0, $ua=0, $nobaody=0)
{
	...
	if ($ua) {
		...
		curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Linux; U; Android 4.0.4; es-mx; HTC_One_X Build/IMM76D) AppleWebKit/534.30 (KHTML, like Gecko) czkVersion/4.0");
	}
	if ($nobaody) {
...
function sysmsg($msg = '未知的异常',$title = 'czk文件目录列表站点提示信息') {
    ?>  
    <!DOCTYPE html>
...
function curl_download($url, $localpath, $timeout = 300)
{
	...
	curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36 czk/2");
	curl_exec($ch);
	if (curl_errno($ch)) {
